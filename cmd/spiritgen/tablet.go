...
func runTablet(args []string) {
	fs := flag.NewFlagSet("tablet", flag.ExitOnError)
	inputFlag := fs.String("input", "", "XLSX 파일 경로 (또는 파일을 실행 파일 위로 드래그 하세요)")
	outputName := fs.String("output", "output.pdf", "PDF 출력 파일 이름 (기본: output.pdf)")
	...
	if inputPath == "" {
		log.Fatal("❌ XLSX 파일이 필요합니다. --input <path> 또는 파일을 실행 파일 위로 드래그 해주세요.")
	}
